package Moose::Documents::Chinese::Cookbook::Basics::Point_AttributesAndSubclassing;
...
這是經典的範例「點」。這直接取自 Perl 6 啟示錄 12 章，和 C 語言經典 K&R 中的範例也相似。


如同所有的 Perl 5 類別，一個 Moose 類別定義在一個套件內。
Moose 會幫我們打開 C<strict> 和 C<warnings> 編譯指令，所以我們只需說 C<use Moose>，and no kittens will die。

當 Moose 載入後，它在你的套件內注入一組語法糖衣函式。
這意味著這些函式被當做 Moose 關鍵字。
它們不是語言的關鍵字，只是些注入你套件的 Perl 函式。

Moose 自動讓你的套件成為 L<Moose::Object> 的子類別。
L<Moose::Object> 類別為屬性提供了建構子，和其他功能。
細節請參考 L<Moose::Object>。

現在這些關鍵字中，我們第一個看到的是 C<has>, C<has>定義了class的instance attribute：

  has 'x' => (isa => 'Int', is => 'rw', required => 1);

這行程式會創立一個名叫C<x>的 attribute。 C<isa> 這個 parameter 代表我們限制用C<Int>型態來傳值（1）。
這個由 accessor 產生的 attibute 不僅容易讀而且容易寫。
...
至此我們介紹如何定義 attribute，接下來我們介紹如何定義 method。在Moose之中定義 method 就像在 Perl 5 的 OO 定義 subroutine一樣:
