...
按照 CPAN 地圖服務 http://mapofcpan.org/ 顯示，
...
仿照日本 Perl 協會將 Moose 相關文件翻譯為日文的作法，
將 Moose 文件翻譯為中文。
但由於事務繁忙而沒有下文。
...
Moose::Class 或簡稱 Class。譯作「類別」。

=item Role 

Moose::Role 或簡稱 Role。譯作「角色」。 
...
摧毀物件並回收記憶體的函式，譯作「解構子」。

=item package

Perl 5 的關鍵字 package，譯作「套件」。
